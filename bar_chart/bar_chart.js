...
d3.csv("data/data_bar_new.csv").then(function(d, i)
 {
  var data = d;
  var keys = [];
  ...
  //     d[j][keys[i]] = parseInt(d[j][keys[i]]);
  //   }
  //
  // }
  // return d;
  ...
  //   if (error) throw error;
    ...
    // d3.csv("data/data_bar_new.csv").then(function(d) {
    //   console.log(d);
    //   for (var i = 1, n=5; i < n; ++i) d[i] = +d[i];
    //
    //   return d;
    // }, function(error, data) {
    //   if (error) throw error;

    x0.domain(d.map(function(d) { return d.Onderwijsniveau; }));
    ...
    g.append("g")
      ...
      .data(function(d) {
        ...
        for(key in d) {
          console.log(key);
          console.log({key: key, value: parseInt(d[key])});
